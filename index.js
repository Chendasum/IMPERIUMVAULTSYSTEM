...
// Enhanced webhook endpoint with FIXED timeout protection for Sum Chenda
app.post("/webhook", async (req, res) => {
    const startTime = Date.now();
    console.log("📨 Webhook received from Telegram");
    
    // 🔥 CRITICAL FIX: Respond to Telegram IMMEDIATELY to prevent timeout
    res.status(200).json({
        status: "received",
        timestamp: new Date().toISOString(),
        enhancedSystem: true
    ...
    // 🔥 FIXED: Process in background with timeout protection
    setImmediate(async () => {
        try {
            console.log(`Processing update ${req.body.update_id}...`);
            
            // Create timeout promise (25 seconds max)
            const timeoutPromise = new Promise((resolve) => {
                setTimeout(() => {
                    console.log("⚠️ Webhook processing timeout (25s)");
                    resolve({ timeout: true });
                }, 25000);
            });
            
            // Create processing promise
            const processPromise = bot.processUpdate(req.body);
            ...
            if (result?.timeout) {
                console.log(`⚠️ Webhook timed out: ${processingTime}ms`);
                
                // Log timeout for Sum Chenda's monitoring
                try {
                    await updateSystemMetrics({
                        webhook_timeouts: 1,
                        timeout_processing_time: processingTime
                    });
                } catch (metricsError) {
                ...
                console.log(`✅ Webhook processed successfully: ${processingTime}ms`);
            ...
            // Log error for Sum Chenda's debugging
            try {
                await updateSystemMetrics({
                    webhook_errors: 1,
                    error_processing_time: processingTime
                });
            } catch (metricsError) {
                console.log("⚠️ Error metrics logging failed:", metricsError.message);
            }
        }
    });
});

console.log("🔥 WEBHOOK FIXED: Immediate response + background processing");
console.log("🔥 WEBHOOK FIXED: Enhanced error handling and timeout protection");
console.log("✅ All webhook timeout issues should be resolved");
