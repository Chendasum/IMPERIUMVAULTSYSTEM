...
// 🔧 ADD THESE MISSING CORE AI FUNCTIONS TO YOUR INDEX.JS

/**
 * Universal Analysis - Routes to best AI model
 */
async function getUniversalAnalysis(prompt, options = {}) {
    const startTime = Date.now();
    try {
        console.log("🤖 Universal Analysis:", prompt.substring(0, 50));
        ...
        // Use GPT-5 for universal analysis
        const response = await openai.chat.completions.create({
            model: model,
            messages: [
                {
                    role: "system",
                    content: "You are an advanced AI assistant with expertise in financial analysis, strategic thinking, and comprehensive problem-solving. Provide detailed, actionable insights."
                },
                {
                    role: "user", 
                    content: prompt
        ...
        console.log(`✅ Universal Analysis completed in ${responseTime}ms`);
        return result;
...
/**
 * Claude Analysis - Direct Claude Opus 4.1 calls
 */
async function getClaudeAnalysis(prompt, options = {}) {
    try {
        console.log("🧠 Claude Analysis:", prompt.substring(0, 50));
        
        // Check if you have Anthropic client configured
...
/**
 * Dual Analysis - Uses both GPT-5 and Claude
 */
async function getDualAnalysis(prompt, options = {}) {
    const startTime = Date.now();
    try {
        console.log("🤖🧠 Dual Analysis:", prompt.substring(0, 50));
        ...
        let response = "🤖🧠 **Dual AI Analysis: GPT-5 + Claude**\n\n";
        
        // GPT-5 Analysis
        if (gptResult.status === 'fulfilled') {
            response += "**GPT-5 Analysis:**\n";
            response += gptResult.value + "\n\n";
        } else {
            response += "**GPT-5 Analysis:** ❌ " + gptResult.reason.message + "\n\n";
        }
        ...
        if (claudeResult.status === 'fulfilled') {
            ...
            response += "**Claude Analysis:** ❌ " + claudeResult.reason.message + "\n\n";
        }
        
        // Synthesis if both succeeded
        if (gptResult.status === 'fulfilled' && claudeResult.status === 'fulfilled') {
            const synthesis = await synthesizeDualAnalysis(gptResult.value, claudeResult.value);
        ...
        console.log(`✅ Dual Analysis completed in ${responseTime}ms`);
        return response;
...
/**
 * Synthesis of dual analysis results
 */
async function synthesizeDualAnalysis(gptAnalysis, claudeAnalysis) {
    try {
        const synthesisPrompt = `Based on these two AI analyses of the same topic, provide a brief synthesis highlighting:
1. Key agreements between the analyses
2. Any unique insights from each AI
3. Overall consensus and actionable conclusions

GPT-5 Analysis: ${gptAnalysis.substring(0, 500)}

Claude Analysis: ${claudeAnalysis.substring(0, 500)}
...
Provide a concise synthesis in 3-4 sentences.`;
        ...
        return "Synthesis not available due to: " + error.message;
    }
}

/**
 * Smart Query Router - Determines best AI for query type
 */
async function routeQuery(query, chatId = null) {
    try {
        ...
        if (queryLower.includes('analyze') || queryLower.includes('strategy') || 
            queryLower.includes('comprehensive') || queryLower.includes('detailed')) {
            return {
                primaryAI: 'DUAL',
                reasoning: 'Complex analysis requires both AI perspectives',
                confidence: 0.9
            };
        }
        
        if (queryLower.includes('code') || queryLower.includes('technical') || 
            queryLower.includes('implementation')) {
            return {
                primaryAI: 'GPT5',
                reasoning: 'Technical queries best handled by GPT-5',
                confidence: 0.8
            };
        }
        ...
        if (queryLower.includes('creative') || queryLower.includes('story') || 
            ...
            return {
                primaryAI: 'CLAUDE',
                reasoning: 'Creative tasks suited for Claude',
                confidence: 0.8
            };
        }
        
        if (queryLower.includes('market') || queryLower.includes('financial') || 
            queryLower.includes('economic')) {
            return {
                primaryAI: 'DUAL',
                reasoning: 'Financial analysis benefits from dual perspective',
                confidence: 0.85
            };
        }
        ...
        return {
            primaryAI: 'GPT5',
            reasoning: 'General query defaulting to GPT-5',
            confidence: 0.7
        };
...
/**
 * Execute Dual Command - Main routing function
 */
async function executeDualCommand(query, chatId, context = {}) {
    const startTime = Date.now();
    try {
        console.log("🚀 Executing dual command:", query.substring(0, 50));
        
        // Get routing decision
        const routing = await routeQuery(query, chatId);
        
        let result;
        let aiUsed;
        
        // Execute based on routing
        switch (routing.primaryAI) {
            case 'DUAL':
                result = await getDualAnalysis(query, context);
                aiUsed = 'GPT5+CLAUDE';
                break;
                
            case 'CLAUDE':
                result = await getClaudeAnalysis(query, context);
                aiUsed = 'CLAUDE';
                break;
                
            case 'GPT5':
            default:
                result = await getUniversalAnalysis(query, context);
                aiUsed = 'GPT5';
                break;
        }
        ...
        // Emergency fallback
        try {
            const fallback = await getUniversalAnalysis(query, { model: "gpt-4o" });
            return {
                response: fallback,
                aiUsed: 'GPT4_FALLBACK',
                success: true,
                responseTime: responseTime,
                error: error.message,
                fallback: true
            ...
            return {
                response: `AI system temporarily unavailable: ${error.message}`,
                aiUsed: 'ERROR',
                success: false,
                responseTime: responseTime,
                error: error.message,
