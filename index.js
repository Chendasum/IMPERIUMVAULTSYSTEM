...
// 🧠 MEMORY STATS COMMAND - Add this missing handler
if (text === '/memory_stats' || text === '/memory') {
    try {
        await bot.sendMessage(chatId, "🧠 Analyzing your strategic memory intelligence...");
        ...
        const [
            ...
            getConversationStats().catch(() => ({ totalConversations: 0 })),
            getUserProfileDB(chatId).catch(() => null),
            getPersistentMemoryDB(chatId).catch(() => []),
            getTrainingDocumentsDB(chatId).catch(() => []),
            getTradingPatterns(chatId).catch(() => [])
        ]);
        
        // Calculate relationship duration
        const relationshipDays = userProfile ? 
            Math.floor((new Date() - new Date(userProfile.first_seen)) / (1000 * 60 * 60 * 24)) : 0;
        
        let memoryReport = `🧠 **YOUR STRATEGIC MEMORY INTELLIGENCE**\n\n`;
        
        // Overall Intelligence Level
        const totalConversations = userProfile?.conversation_count || 0;
        let intelligenceLevel = 'BASIC';
        if (totalConversations > 500) intelligenceLevel = 'MASTER';
        else if (totalConversations > 100) intelligenceLevel = 'EXPERT';
        else if (totalConversations > 50) intelligenceLevel = 'ADVANCED';
        else if (totalConversations > 10) intelligenceLevel = 'INTERMEDIATE';
        
        memoryReport += `⚡ **INTELLIGENCE LEVEL: ${intelligenceLevel}**\n\n`;
        
        // Memory Statistics
        memoryReport += `📊 **MEMORY STATISTICS:**\n`;
        memoryReport += `• Total Conversations: ${totalConversations}\n`;
        memoryReport += `• Persistent Memories: ${persistentMemory.length}\n`;
        memoryReport += `• Training Documents: ${trainingDocs.length}\n`;
        memoryReport += `• Trading Patterns: ${tradingPatterns.length}\n`;
        memoryReport += `• Relationship Duration: ${relationshipDays} days\n\n`;
        
        // User Profile Analysis
        if (userProfile) {
            memoryReport += `👤 **YOUR STRATEGIC PROFILE:**\n`;
            memoryReport += `• First Interaction: ${new Date(userProfile.first_seen).toLocaleDateString()}\n`;
            memoryReport += `• Last Active: ${new Date(userProfile.last_seen).toLocaleDateString()}\n`;
            memoryReport += `• Communication Style: Strategic Command Authority\n`;
            ...
            if (userProfile.preferences && Object.keys(userProfile.preferences).length > 0) {
                ...
                memoryReport += `• Saved Preferences: ${Object.keys(prefs).length} items\n`;
            }
            memoryReport += `\n`;
        }
        ...
        if (persistentMemory.length > 0) {
            memoryReport += `🧠 **STRATEGIC MEMORIES (Top 5):**\n`;
            persistentMemory.slice(0, 5).forEach((memory, index) => {
                ...
                const emoji = importance === 'critical' ? '🔴' : 
                             importance === 'high' ? '🟡' : '🟢';
                memoryReport += `${index + 1}. ${emoji} ${fact.substring(0, 80)}...\n`;
            });
            memoryReport += `\n`;
        }
        ...
        if (trainingDocs.length > 0) {
            const totalWords = trainingDocs.reduce((sum, doc) => sum + (doc.word_count || 0), 0);
            memoryReport += `📚 **KNOWLEDGE BASE:**\n`;
            memoryReport += `• Documents: ${trainingDocs.length}\n`;
            memoryReport += `• Total Words: ${totalWords.toLocaleString()}\n`;
            memoryReport += `• Recent Upload: ${trainingDocs[0]?.file_name || 'None'}\n\n`;
        }
        
        // Trading Patterns
        if (tradingPatterns.length > 0) {
            memoryReport += `💹 **TRADING INTELLIGENCE:**\n`;
            tradingPatterns.slice(0, 3).forEach((pattern, index) => {
                memoryReport += `${index + 1}. ${pattern.description} (${pattern.confidence}% confidence)\n`;
            });
            memoryReport += `\n`;
        }
        
        // Intelligence Growth Recommendations
        memoryReport += `🚀 **INTELLIGENCE GROWTH:**\n`;
        if (totalConversations < 10) {
            memoryReport += `• Continue chatting to build strategic profile\n`;
            memoryReport += `• Share your investment preferences\n`;
            memoryReport += `• Upload financial documents for AI training\n`;
        } else if (totalConversations < 50) {
            memoryReport += `• Upload trading strategies for pattern recognition\n`;
            memoryReport += `• Share Cambodia deal preferences\n`;
            memoryReport += `• Discuss your risk management approach\n`;
        } else {
            memoryReport += `• Your AI has strong strategic intelligence\n`;
            memoryReport += `• Continue conversations for expert-level insights\n`;
            memoryReport += `• Share complex scenarios for advanced learning\n`;
        }
        
        memoryReport += `\n💡 **Memory Commands:**\n`;
        memoryReport += `• /documents - View training documents\n`;
        memoryReport += `• /portfolio - Cambodia fund intelligence\n`;
        memoryReport += `• /trading - Trading pattern analysis\n`;
        
        await sendSmartResponse(bot, chatId, memoryReport, "Strategic Memory Intelligence", 'analysis');
        
    } catch (error) {
        await sendSmartResponse(bot, chatId, `❌ **Memory Analysis Error:** ${error.message}\n\nYour memory system is active, but there may be a database connection issue.`, null, 'general');
    }
    return;
}

// 📚 TRAINING DOCUMENTS COMMAND - Also add this if missing
if (text === '/documents' || text === '/training_docs' || text === '/files') {
    try {
        await bot.sendMessage(chatId, "📚 Retrieving your strategic training documents...");
        
        const docs = await getTrainingDocumentsDB(chatId);
        
        if (docs.length === 0) {
            const helpMessage = `📚 **No Strategic Training Documents Found**\n\n` +
                `💡 **How to Add Documents:**\n` +
                `• Upload any file (.txt, .pdf, .docx)\n` +
                `• Add caption: "train" or "database"\n` +
                `• AI will save it for strategic reference\n\n` +
            ...
            await sendSmartResponse(bot, chatId, helpMessage, "Strategic Training Documents", 'general');
            return;
        }
...
// 💹 TRADING PATTERNS COMMAND - Also add this
if (text === '/trading_patterns' || text === '/patterns') {
    try {
        await bot.sendMessage(chatId, "💹 Analyzing your strategic trading patterns...");
        
        const patterns = await getTradingPatterns(chatId);
        
        if (patterns.length === 0) {
            const helpMessage = `💹 **No Trading Patterns Detected Yet**\n\n` +
                `💡 **How to Build Trading Intelligence:**\n` +
                `• Discuss your trading strategies\n` +
                `• Share your risk management rules\n` +
                `• Talk about your position sizing\n` +
                `• Explain your market preferences\n\n` +
                `🎯 The AI will automatically identify and save your patterns!`;
            
            await sendSmartResponse(bot, chatId, helpMessage, "Trading Pattern Analysis", 'trading');
            return;
        }
        
        let response = `💹 **Your Strategic Trading Patterns (${patterns.length}):**\n\n`;
        patterns.forEach((pattern, i) => {
            response += `**${i + 1}. ${pattern.type}**\n`;
            response += `• 📝 Pattern: ${pattern.description}\n`;
            response += `• 🎯 Confidence: ${pattern.confidence}%\n`;
            response += `• 📅 Detected: ${new Date(pattern.first_detected).toLocaleDateString()}\n`;
            response += `• ✅ Confirmed: ${new Date(pattern.last_confirmed).toLocaleDateString()}\n\n`;
        });
        
        response += `💡 **Strategic Usage:** Your AI uses these patterns to provide personalized trading advice!`;
        
        await sendSmartResponse(bot, chatId, response, "Strategic Trading Patterns", 'trading');
        
    } catch (error) {
        await sendSmartResponse(bot, chatId, `❌ Error retrieving trading patterns: ${error.message}`, null, 'general');
    }
    return;
}
