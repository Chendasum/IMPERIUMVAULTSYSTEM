...
// ---- Added: Shared Pool + Robust SSL (Railway-ready) ----
const DB_URL = process.env.DATABASE_URL || '';
const isProd = process.env.NODE_ENV === 'production';
const forceSSL = process.env.FORCE_DB_SSL === '1';

const SHARED_POOL_CONFIG = {
    connectionString: DB_URL,
    ssl: (isProd || forceSSL) ? { rejectUnauthorized: false } : false,
...
let __sharedPool = null;
function getPool() {
    if (!DB_URL) {
        console.warn('⚠️ DATABASE_URL is missing — DB features disabled');
        return null;
    }
    if (!__sharedPool) {
        __sharedPool = new (require('pg').Pool)(SHARED_POOL_CONFIG);
        __sharedPool.on('error', (err) => console.error('❌ PG Pool Error:', err.message));
    }
    return __sharedPool;
}
// ---- End Added ----
...
/**
 * 📊 Lightweight DB stats for /status or /dbping
 */
...
/**
 * 🏁 Auto-initialize using shared pool (safe to call from app start)
 */
async function initializeDatabaseAuto() {
    const pool = getPool();
    if (!pool) {
        console.warn('⚠️ initializeDatabaseAuto skipped — no DATABASE_URL');
        return;
    }
    await initializeDatabase(pool);
}
