...
function analyzeQuery(userMessage, messageType = 'text', hasMedia = false, memoryContext = null) {
    ...
    let gpt5Config = {
        ...
        verbosity: 'medium',
        max_completion_tokens: 8000,        // FIXED: Use consistent parameter
        temperature: 0.7,
        priority: 'standard',
    ...
    if (speedPatterns.some(pattern => pattern.test(message))) {
        gpt5Config = {
            ...
            verbosity: 'low',
            max_completion_tokens: 6000,    // FIXED: Use consistent parameter
            priority: 'speed',
    ...
    // COMPLEX ANALYSIS - Full GPT-5 (maximum intelligence)
        ...
        gpt5Config = {
            ...
            verbosity: 'high',
            max_completion_tokens: 16000,    // FIXED: Use consistent parameter
            temperature: 0.6,
            priority: 'complex',
    ...
    // MATH/CODING - Full GPT-5 with maximum precision
        ...
        gpt5Config = {
            ...
            reasoning_effort: 'high',
            verbosity: 'medium',
            max_completion_tokens: 12000,    // FIXED: Use consistent parameter
            temperature: 0.3,
    ...
    // CAMBODIA/REGIONAL - GPT-5 Mini (cost efficient)
        ...
        gpt5Config = {
            ...
            reasoning_effort: 'medium',
            verbosity: 'high',
            max_completion_tokens: 10000,    // FIXED: Use consistent parameter
            temperature: 0.6,
            priority: 'regional',
...
// 🚀 DIRECT GPT-5 EXECUTION (FIXED with consistent parameters)
async function executeThroughGPT5System(userMessage, queryAnalysis, context = null, memoryData = null, chatId = null) {
    try {
        ...
        if (queryAnalysis.memoryImportant && context && context.length > 0) {
            // FIXED: Limit memory context size to prevent token overflow
        ...
        // Add specific memory data with size limits (FIXED)
        if (memoryData) {
        ...
        // FIXED: Build consistent options object for all models
        const options = {
            model: queryAnalysis.gpt5Model,
        ...
        if (queryAnalysis.gpt5Model === 'gpt-5-chat-latest') {
            // Chat model uses temperature but still uses max_completion_tokens
            if (queryAnalysis.temperature) options.temperature = queryAnalysis.temperature;
        } else {
            // Reasoning models use additional parameters
...
async function checkGPT5OnlySystemHealth() {
    ...
    for (const { name, model, description } of gpt5Models) {
        try {
            // FIXED: Use consistent max_completion_tokens for all models
            const options = {
                model: model,
                max_completion_tokens: 50  // Minimum test size
            };
...
// Continue with the rest of your utility functions...
...
module.exports = {
    ...
    testMemoryIntegration: async (chatId) => {
        ...
        return { status: 'GPT-5 system ready', tests: { gpt5System: true } };
    },
    getCurrentCambodiaDateTime,
    ...
    getSystemAnalytics: () => ({
        version: '5.0 - GPT-5 Only Optimization (FIXED)',
        architecture: 'index.js → dualCommandSystem.js → openaiClient.js',
        parameterConsistency: 'Fixed - All models use max_completion_tokens',
        memoryLimits: 'Added context size controls',
        fallbackSystem: 'Circuit breaker implemented'
    })
};

console.log('✅ FIXED GPT-5 Only System loaded (v5.0 - Parameter Consistency Fixed)');
console.log('🔧 All models now use max_completion_tokens parameter consistently');
console.log('💾 Memory context size limits added to prevent token overflow');
console.log('🛡️ Circuit breaker fallback system implemented');
