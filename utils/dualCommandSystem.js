...
/**
 * Operator Mode v3 (Aug 2025)
 * This wrapper upgrades the old dualCommandSystem into an actionable operator without deleting the original file.
 * We keep the legacy code intact below and expose new execution functions that call into GPT‑5 and Telegram
 * while supporting planning, confirmation, and auditing.
 *
 * IMPORTANT: Original code is preserved in the LEGACY section after this wrapper.
 */

const crypto = require('crypto');
let openaiClient, telegramSplitter, database, memory;
try { openaiClient = require('./openaiClient'); } catch (e) { openaiClient = { getGPT5Analysis: async (p)=>String(p) }; }
try { telegramSplitter = require('./telegramSplitter'); } catch (e) { telegramSplitter = { sendGPTResponse: async ()=>true, sendAlert: async ()=>true, sendAnalysis: async ()=>true }; }
try { database = require('./database'); } catch (e) { database = { saveConversationEmergency: async ()=>true, savePersistentMemory: async ()=>true }; }
try { memory = require('./memory'); } catch (e) { memory = { buildConversationContext: async ()=>'' }; }

const OPERATOR = {
  SYSTEM_NAME: process.env.SYSTEM_NAME || 'Imperium Operator',
  REQUIRE_CONFIRMATION: process.env.OPERATOR_REQUIRE_CONFIRMATION !== '0',
  ALLOW_FORCE: process.env.OPERATOR_ALLOW_FORCE === '1',
  MAX_STEPS: Number(process.env.OPERATOR_MAX_STEPS || 8),
  MAX_ACTION_TIME_MS: Number(process.env.OPERATOR_MAX_ACTION_TIME_MS || 30000),
  DEFAULT_MODEL: process.env.OPERATOR_DEFAULT_MODEL || 'gpt-5-mini',
  DEFAULT_REASONING: process.env.OPERATOR_DEFAULT_REASONING || 'medium',
  DEFAULT_MAX_TOKENS: Number(process.env.OPERATOR_DEFAULT_MAX_TOKENS || 2000),
  TZ: process.env.OPERATOR_TZ || 'Asia/Phnom_Penh'
};

function newTrace(){ return crypto.randomUUID(); }
function nowIso(){ return new Date().toISOString(); }
function idempotencyKey(s){ return crypto.createHash('sha1').update(String(s)).digest('hex'); }

function withTimeout(p, ms, label='action'){
  let to; const t = new Promise((_,rej)=>to=setTimeout(()=>rej(new Error(`${label} timeout after ${ms}ms`)), ms));
  return Promise.race([p, t]).finally(()=>clearTimeout(to));
}

function chooseModel(msg){
  const t = String(msg||'').toLowerCase();
  const speed = /(urgent|asap|quick|fast|now)/i.test(t);
  const complex = /(strategy|framework|in-depth|analyze|plan)/i.test(t);
  if (speed) return { model:'gpt-5-nano', reasoning_effort:'low', max_completion_tokens:4000, temperature:0.6 };
  if (complex) return { model:'gpt-5', reasoning_effort:'high', max_completion_tokens:12000, temperature:0.5 };
  return { model:'gpt-5-mini', reasoning_effort:'medium', max_completion_tokens:8000, temperature:0.6 };
}

// Minimal planner using GPT‑5 returning JSON
const PLAN_PROMPT = (input, context, tz) => `You are an execution planner for an operator bot. Return STRICT JSON only.\nSchema:{\"intent\":string,\"dangerous\":boolean,\"confirmRequired\":boolean,\"steps\":[{\"action\":string,\"args\":object}],\"notes\":string|null}\nTZ:${tz}\nContext:${(context||'').slice(0,800)}\nInput:${input}`;

async function plan(input, context=''){
  const sel = chooseModel(input);
  const out = await openaiClient.getGPT5Analysis(PLAN_PROMPT(input, context, OPERATOR.TZ), {
    model: sel.model, reasoning_effort: sel.reasoning_effort, max_completion_tokens: 900, temperature: 0
  });
  try { return JSON.parse(out); } catch(_){ const m = out.match(/\{[\s\S]*\}/); return m?JSON.parse(m[0]):{ intent:'analyze', dangerous:false, confirmRequired:false, steps:[], notes:'parse_failed' }; }
}

const Actions = {
  async send(ctx, args){
    const text = String(args?.text||'').trim();
    if (!text) throw new Error('send.text required');
    if (telegramSplitter?.sendGPTResponse) return withTimeout(telegramSplitter.sendGPTResponse(ctx.bot, ctx.chatId, text, 'Operator Message', { traceId: ctx.traceId }), OPERATOR.MAX_ACTION_TIME_MS, 'telegram.send');
    if (ctx.bot?.sendMessage) return withTimeout(ctx.bot.sendMessage(ctx.chatId, text), OPERATOR.MAX_ACTION_TIME_MS, 'bot.sendMessage');
    return true;
  },
  async save(ctx, args){
    const fact = String(args?.fact||args?.text||'').trim();
    if (!fact) throw new Error('save.fact required');
    return withTimeout(database.savePersistentMemory(ctx.chatId, { fact, createdAt: nowIso(), traceId: ctx.traceId }), OPERATOR.MAX_ACTION_TIME_MS, 'db.save');
  },
  async analyze(ctx, args){
    const sel = chooseModel(args?.text||ctx.raw);
    const out = await openaiClient.getGPT5Analysis(args?.text||ctx.raw, { model: sel.model, reasoning_effort: sel.reasoning_effort, max_completion_tokens: sel.max_completion_tokens, temperature: sel.temperature });
    if (telegramSplitter?.sendGPTResponse) await telegramSplitter.sendGPTResponse(ctx.bot, ctx.chatId, out, 'Analysis', { traceId: ctx.traceId });
    return out;
  },
  async health(ctx){
    const h = await openaiClient.checkGPT5SystemHealth();
    const s = `🩺 Health: ${h.overallHealth?'OK':'Attention'} | gpt5:${h.gpt5Available?'✅':'❌'} mini:${h.gpt5MiniAvailable?'✅':'❌'} nano:${h.gpt5NanoAvailable?'✅':'❌'} fallback:${h.fallbackWorking?'✅':'❌'}`;
    if (telegramSplitter?.sendGPTResponse) await telegramSplitter.sendGPTResponse(ctx.bot, ctx.chatId, s, 'Health');
    return h;
  }
};

function needsConfirmation(plan, envelope){
  if (envelope.force && OPERATOR.ALLOW_FORCE) return false;
  if (!OPERATOR.REQUIRE_CONFIRMATION) return false;
  return Boolean(plan.dangerous || plan.confirmRequired);
}

function buildEnvelope(rawText, ctx={}){
  const text = String(rawText||'').trim();
  const force = /\b--force\b/i.test(text);
  const confirm = /\b--confirm\b|\bok\b|\byes\b/i.test(text);
  const isSlash = text.startsWith('/');
  const intent = isSlash && /^\/(send|save|broadcast|health|plan|op|run|intel)\b/i.test(text)
    ? text.slice(1).split(/\s+/)[0].toLowerCase()
    : null;
  return {
    traceId: ctx.traceId || newTrace(),
    msgId: ctx.msgId || idempotencyKey(text),
    ts: nowIso(),
    type: intent ? 'COMMAND' : 'QUESTION',
...
async function executePlan(envelope, plan, runtime){
  const ctx = { ...runtime, traceId: envelope.traceId, raw: envelope.raw };
  const steps = Array.isArray(plan.steps) ? plan.steps.slice(0, OPERATOR.MAX_STEPS) : [];
  const results = [];
  for (let i=0;i<steps.length;i++){
    const s = steps[i]; const fn = Actions[s.action];
    if (typeof fn !== 'function') throw new Error(`Unknown action: ${s.action}`);
    const t0 = Date.now(); const r = await fn(ctx, s.args||{});
    results.push({ i, action: s.action, ok: !!r, ms: Date.now()-t0 });
  }
  return { ok: results.every(x=>x.ok), results };
}

async function executeOperatorCommand(rawInput, chatId, bot=null, options={}){
  const t0 = Date.now();
  const traceId = options.traceId || newTrace();
  const msgId = options.msgId || idempotencyKey(`${chatId}:${rawInput}`);
  const env = buildEnvelope(rawInput, { traceId, msgId });
  let memCtx = ''; try { memCtx = await memory.buildConversationContext(chatId); } catch {}
  const planObj = env.intent === 'analyze'
    ? await plan(env.raw, memCtx)
    : (env.intent === 'health'
        ? { intent:'health', dangerous:false, confirmRequired:false, steps:[{ action:'health', args:{} }] }
        : { intent:'analyze', dangerous:false, confirmRequired:false, steps:[{ action:'analyze', args:{ text: env.raw } }] });
  if (needsConfirmation(planObj, env) && !env.confirm){
    const txt = `⚠️ ${OPERATOR.SYSTEM_NAME} needs confirmation to proceed.\nIntent:${planObj.intent}\nSteps:${planObj.steps?.length||0}\nReply /run --confirm`;
    if (telegramSplitter?.sendGPTResponse) await telegramSplitter.sendGPTResponse(bot, chatId, txt, 'Confirmation', { traceId });
    return { success:false, requiresConfirmation:true, traceId, msgId, intent: planObj.intent };
  }
  const result = await executePlan(env, planObj, { bot, chatId });
  if (database?.saveConversationEmergency){
    try { await database.saveConversationEmergency(chatId, rawInput, JSON.stringify({ plan:planObj, result }), { operator:true, traceId, msgId }); } catch {}
  }
  const done = `✅ ${OPERATOR.SYSTEM_NAME} executed ${planObj.steps.length} step(s).`;
  if (telegramSplitter?.sendGPTResponse) await telegramSplitter.sendGPTResponse(bot, chatId, done, 'Operator', { traceId });
  return { success:true, operator:true, plan:planObj, result, traceId, msgId, ms: Date.now()-t0 };
}

// Minimal public exports for the wrapper (the legacy file may export more later)
module.exports.__OPERATOR_WRAPPER__ = {
  executeOperatorCommand,
  chooseModel,
  plan
};

/* ==== END OF OPERATOR WRAPPER. ORIGINAL FILE BELOW (PRESERVED) ==== */


// ==== LEGACY dualCommandSystem.js (PRESERVED, UNCHANGED) ====
