'use strict';
// telegramSplitter.js — 10/10 Production v3.1 (Ultra‑Compat, Node-safe)
// - CommonJS, ASCII-only; NO optional chaining, NO RegExp look‑behind
// - Safe Markdown V1 sanitizer + auto HTML fallback on entity errors
// - Smart 4096 split (paragraph/line/word) with code-fence awareness + Part X/Y
// - Per-chat + global throttles; rate-limit retries (honors retry_after)
// - Circuit breaker + metrics; queueing for bursty sends
// - Rich helpers: setupTelegramHandler, sendGPTResponse, sendSystemStatus, sendError, sendSuccess
// - Legacy adapter keeps old calls working: sendGPT5 / sendGPT5Mini / sendGPT5Nano / sendGPT5Chat / sendGPTResponse / sendAlert
...
var CONFIG = {
  ...
  SOFT_SPLIT: 3800,           // aim before hard limit
  PREF_CHUNK: 3500,           // preferred chunk size

  // Delivery / throttles
  PER_CHAT_MIN_MS: 1100,      // ~1 msg/sec per chat
  GLOBAL_MIN_MS: 33,          // tiny global spacing
  MAX_RETRIES: 3,
  RETRY_BASE_MS: 800,

  // Formatting
  PARSE_MODE: 'Markdown',     // V1 for broadest compatibility
  AUTO_EMOJI: true,
  SMART_SPACING: true,
...
function sanitizeForTelegramMarkdown(text) {
  ...
  // Escape unmatched * and _ (simple heuristic for Markdown V1)
  text = text.replace(/([*_])(?![^\1]*\1)/g, '\\\\$1');

  // Restore blocks exactly
  var i = 0;
...
function formatMarkdownBasic(text) {
  ...
  // *italic* (single asterisks) -> _italic_ (no look-behind)
  text = text.replace(/(^|[^*])\*([^*\n]+)\*(?!\*)/g, '$1_$2_');
  // ~~strike~~ -> ~strike~
  text = text.replace(/~~([^~]+)~~/g, '~$1~');
  return text;
...
function TelegramMessageHandler(bot, defaultOptions) {
  ...
  this.queueDelay = 300;
  console.log('\u2705 Telegram message handler v3.1 ready');
}
...
function demonstrateEnhancedTelegram(bot) {
  ...
  return handler.sendGPTResponse(gptResponse, { model: 'gpt-5-mini', tokens: 1250, cost: 0.00125, executionTime: 2340, costTier: 'economy' }, 'demo_chat');
}

// -----------------------------------------------------------------------------
// Legacy adapter (drop-in for older code paths)
// -----------------------------------------------------------------------------
function _handlerFrom(bot) { return setupTelegramHandler(bot); }

var legacyAdapter = {
  sendGPT5: function (bot, chatId, response, meta) {
    var h = _handlerFrom(bot);
    meta = meta || {}; meta.model = meta.model || 'gpt-5';
    return h.sendGPTResponse(String(response), meta, chatId);
  },
  sendGPT5Mini: function (bot, chatId, response, meta) {
    var h = _handlerFrom(bot);
    meta = meta || {}; meta.model = meta.model || 'gpt-5-mini';
    return h.sendGPTResponse(String(response), meta, chatId);
  },
  sendGPT5Nano: function (bot, chatId, response, meta) {
    var h = _handlerFrom(bot);
    meta = meta || {}; meta.model = meta.model || 'gpt-5-nano';
    return h.sendGPTResponse(String(response), meta, chatId);
  },
  sendGPT5Chat: function (bot, chatId, response, meta) {
    var h = _handlerFrom(bot);
    meta = meta || {}; meta.model = meta.model || 'gpt-5-chat-latest';
    return h.sendGPTResponse(String(response), meta, chatId);
  },
  sendGPTResponse: function (bot, chatId, response, meta) {
    var h = _handlerFrom(bot);
    return h.sendGPTResponse(String(response), meta || {}, chatId);
  },
  sendAlert: function (bot, chatId, errorResponse, title) {
    var h = _handlerFrom(bot);
    var err = (errorResponse instanceof Error) ? errorResponse : new Error(String(errorResponse));
    return h.sendError(err, title || 'System Error', chatId);
  }
};
...
module.exports = {
  ...
  // legacy adapter used by dualcommandsystem.js
...
if (require.main === module) {
  var mockBot = { sendMessage: function (chatId, text, options) { console.log('\ud83e\udd16 MockBot -> ' + chatId + ': ' + (text.length > 60 ? text.slice(0, 60) + '...' : text)); return new Promise(function (res) { setTimeout(function () { res({ message_id: Math.floor(Math.random() * 100000) }); }, 80); }); } };
  demonstrateEnhancedTelegram(mockBot).then(function () { console.log('\n\ud83c\udf89 Demo complete'); }).catch(function (e) { console.error('Demo failed:', e); });
}
