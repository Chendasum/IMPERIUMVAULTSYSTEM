...
async function getChatWithMemory(messages, memory, options) {
  ...
  async function withBackoff(fn) {
    ...
    for (var i=0;i<delays.length;i++) {
      try { return await fn(); }
        ...
        var msg = (e && e.response && e.response.data && e.response.data.error && e.response.data.error.message) || (e && e.message) || "";
        var rate = status === 429 || /rate/i.test(String(msg));
        if (!rate) throw e; await new Promise(function(r){ setTimeout(r, delays[i]); });
      }
    }
    return fn();
  }
  ...
  try {
    ...
    var ms2 = Date.now() - start;
    logApiCall(model, apiUsed, inputTokens, outputTokens, ms2, false, err && err.message);
    metrics.recordCall(model, false, 0, 0, ms2, err && err.message);
    return "Service error (chat-with-memory). Details: " + (err && err.message ? err.message : String(err));
  }
}
