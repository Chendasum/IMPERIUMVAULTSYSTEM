...
// Node compatibility: no optional chaining, no nullish coalescing, no object spread.
// Public API preserved + memory helpers.
...
// Safe getter (no optional chaining)
...
var openai = new OpenAI({
  ...
  defaultHeaders: {
    "User-Agent": "IMPERIUM-VAULT-GPT5/2.5.0",
...
function buildResponsesRequest(model, input, options) {
  ...
  if (["low", "medium", "high"].indexOf(effort) >= 0) req.reasoning = { effort: effort };
...
async function getGPT5AnalysisWithMemory(prompt, memory, options) {
  var merged = attachMemoryToPrompt(prompt, memory);
  return getGPT5Analysis(merged, options || {});
}

async function getChatWithMemory(messages, memory, options) {
  options = options || {};
  ...
  try {
    ...
    var completion = await circuitBreaker.execute(function () { return withBackoff(function () { return openai.chat.completions.create(chatReq); }); });
    ...
    return "Service error (chat-with-memory). Details: " + emsg;
  }
}
...
async function checkGPT5SystemHealth() {
  ...
  try { var fb = await openai.chat.completions.create({ model: GPT5_CONFIG.FALLBACK_MODEL, messages: [{ role: "user", content: "ok" }], max_tokens: 5, temperature: 0 }); if (g(fb, "choices.0.message.content", null)) health.fallbackWorking = true; } catch (e) { health.errors.push("Fallback: " + e.message); }
